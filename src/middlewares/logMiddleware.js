...
function logMiddleware (req, res, next){
    if(req.session.usuarioLogueado == undefined) {
        ...
        if(req.originalUrl == "/"){
            ...
            res.render("products/productList",{usuarioLogueado: req.session.usuarioLogueado, products: productsJson})
        }else if(req.originalUrl == "/products/" + req.params.id){
            let idProduct = req.params.id - 1;
            let product = fs.readFileSync(path.join(__dirname, "../data/products.json"), "utf8");
